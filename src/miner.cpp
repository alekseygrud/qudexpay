...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("QDPMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("QDPMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("QDPMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("qdp-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("QDPMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running QDPMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadQDPMiner() exception");
    } catch (...) {
        LogPrintf("ThreadQDPMiner() exception");
    }

    LogPrintf("ThreadQDPMiner exiting\n");
}
