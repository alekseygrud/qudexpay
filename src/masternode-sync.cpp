...
void CMasternodeSync::GetNextAsset()
{
    ...
    case (MASTERNODE_SYNC_INITIAL)://0
    case (MASTERNODE_SYNC_FAILED): // should never be used here actually, use Reset() instead
        ClearFulfilledRequest();
        RequestedMasternodeAssets = MASTERNODE_SYNC_SPORKS;
        break;
    case (MASTERNODE_SYNC_SPORKS)://1
        RequestedMasternodeAssets = MASTERNODE_SYNC_LIST;
        break;
    case (MASTERNODE_SYNC_LIST)://2
        RequestedMasternodeAssets = MASTERNODE_SYNC_MNW;
        break;
    case (MASTERNODE_SYNC_MNW)://3
        RequestedMasternodeAssets = MASTERNODE_SYNC_BUDGET;
        break;
    case (MASTERNODE_SYNC_BUDGET)://4
...
std::string CMasternodeSync::GetSyncStatus()
{
    ...
    case MASTERNODE_SYNC_FAILED:   //998
...
void CMasternodeSync::Process()
{
    ...
    BOOST_FOREACH (CNode* pnode, vNodes) {
        ...
        if (fDebug)
        {
            LogPrintf("CMasternodeSync::Process() - pnode->HasFulfilledRequest(\"getspork\") = %s pnode->nVersion = %d\n", pnode->HasFulfilledRequest("getspork"),pnode->nVersion);
        ...
        if (RequestedMasternodeAssets == MASTERNODE_SYNC_SPORKS) {
            ...
            pnode->PushMessage("getsporks"); //get current network sporks
            if (RequestedMasternodeAttempt >= 2) GetNextAsset();//QQQ
            RequestedMasternodeAttempt++;
