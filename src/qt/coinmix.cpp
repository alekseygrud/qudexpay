...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::QDP)
    {
    }
...
void Coinmix::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("QDP")
    //updateDisplayUnit();
}
...
void Coinmix::updateObfuscationProgress()
{
    ...
    QString strAnonymizeQDPAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeQDPAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeQDPAmount = strAnonymizeQDPAmount.remove(strAnonymizeQDPAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeQDPAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeQDPAmount * COIN) nMaxToAnonymize = nAnonymizeQDPAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeQDPAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeQDPAmount));
        strAnonymizeQDPAmount = strAnonymizeQDPAmount.remove(strAnonymizeQDPAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeQDPAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void Coinmix::ObfuScationStatus()
{
...
void Coinmix::ObfuscationAuto()
{
    ObfuScationPool.DoAutomaticDenominating();
}

void Coinmix::ObfuscationReset()
{
...
void Coinmix::toggleObfuscation()
{
    QSettings settings;
