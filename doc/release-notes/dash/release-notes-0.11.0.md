...
- Reduced lower obfuscation limit to 1.5 DRK
...
- Added more detailed Obfuscation status information to Overview tab
- Added Obfuscation high precision matching engine
- Added Obfuscation balance to `getinfo`
...
- Add '-disablewallet' mode to let qdpd run entirely without wallet (when
  built with wallet)
- Update default '-rpcsslciphers' to include TLSv1.2
- make '-logtimestamps' default on and rework help-message
- RPC client option: '-rpcwait', to wait for server start
- Remove '-logtodebugger'
- Allow `-noserver` with qdpd
