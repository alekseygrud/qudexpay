...
- saction for Obfuscation. These are special transactions that require a signature that only the masternodes can create.
- Obfuscation now has no fees to track what-so-ever, all that will ever be in Obfuscation transactions are Obfuscation denominations.
...
- Split up obfuscation.cpp into masternode.cpp, activemasternode.cpp and instantx.cpp
- Added modular ProcessMessages for Obfuscation, Masternodes and InstantX
...
- Fixed all sorts of issues with collateral and Obfuscation.
...
- Obfuscation tested with 2.5 DRK successfully and I've set that as the minimum
...
- Obfuscation UI is disabled for masternodes now and titlebar says "[masternode]"
...
- Updated wording for obfuscation messages
- Removed disable obfuscation checkbox from config screen
- Added "-enableobfuscation" cli option
...
- Obfuscation should anonymize very close to the intended amount now
- Added Amount/Rounds to overview screen so you can see current settings
- Overview obfuscation cache is cleared on settings change (will instantly update)
...
- This update deals with freezing, slow wallets, slow load times and the
  ...
  Obfuscation process has been reserve keys for every attempt! To rectify this
...
- Easy to read obfuscation progress bar (Mouse over for very detailed info)
...
- Client now creates fee sizes inputs for use in obfuscation denomination phases
...
- Client now prioritises sending denominated funds back into Obfuscation. This will
...
- Obfuscation now checks diskspace before sending anything to a masternode.
...
- Majorly improved the way obfuscation participants are paired together. It should
  be super fast now.
...
- Obfuscation now shows address instead of "n/a"
...
- Added GUI configuration for Obfuscation Rounds, Enable Obfuscation and Anonymize
  amount of DRK
...
- Obfuscation limited to 5000DRK per wallet.dat. Client will warn about this the
  first time it's opened, then disable obfuscation from then on.
...
- Found race condition with new blocks and clearing obfuscation entries that was
...
- Obfuscation Denominate Outputs are now in a random order:
...
- Obfuscation now uses 3 participants instead of two.
- SplitUpMoney can now make collateral inputs when needed
- Transactions now shows obfuscation transaction types for easier understanding
  of what's going on:
...
- Fixed the money destruction bug, it was caused by "obfuscation denominate 8000".
  I missed a reference and the client passed an empty address to
  SendMoneyToDestination. rcp obfuscation source: http://pastebin.com/r14piKuq
...
- Added Obfuscation detection to the UI
- fixed senttoaddress, it will use all inputs when obfuscation is disabled now.
  Otherwise it will ONLY use non-denom.
- "obfuscation addr amount" now returns the hash of the transaction it creates
...
- Removed "obfuscation denominate", obfuscation now will figure out the most it can
  denominate. Use "obfuscation auto" instead.
...
- QT GUI will now ask to unlock the wallet when it detects Obfuscation wants to do
  something and lock it when it's done again.
- Obfuscation is turned off by default in the daemon now. In most cases daemons
  won't want to run with anonymity (pools, exchanges, etc), if a user does they
  can override the default setting with -enabledaemonobfuscation=true
- Fees per round of Obfuscation are 0.001DRK or $0.00538 at current prices. This
...
- RPC calls are changed a bit:
    ...
    obfuscation denominate 100 - Will denominate 100DRK
    obfuscation auto - Will run AutoDenominate
    obfuscation Xaddr 100 - Will send 100 denominated DRK to Xaddr
...
- Fixed obfuscation+ rounds function, it was returning bad data in some cases
  causing obfuscation inputs to never stop being sent through obfuscation.
...
/darkcoin-qt -datadir=/home/user/.darkcoin -listen=0 -obfuscationrounds=8 -usemasternode="192.168.56.102:55600"
/darkcoin-qt -datadir=/home/user/.darkcoin2 -listen=0 -obfuscationrounds=8 -usemasternode="192.168.56.102:55600"

and even disable obfuscation auto-denom if wanted:
/darkcoin-qt -datadir=/home/user/.darkcoin -listen=0 -obfuscationrounds=8 -usemasternode="192.168.56.102:55600" -disableobfuscation=1
...
- Added a smart input splitting method. Place 1000+DRK into a brand new wallet
  and it will be split into many inputs compatible with Obfuscation
...
- ObfuscationRounds had a bug that I fixed, should calculate correctly now
...
- New column "Obfuscation Rounds" in coincontrol to show how secure a given input is
- Fixed a few issues causing obfuscation to fail. We should see many more obfuscations
...
- Redid denominations to 1, 10, 100, and 500. Maybe this is too simple, but it
  ...
  again through obfuscation for the next transactions. We'll see how it works.
...
- Improved ObfuscationInput add entry verification. Masternodes will now reject
