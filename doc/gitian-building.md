...
*Setup instructions for a gitian build of QDP using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the QDP
...
Clone the git repositories for qdp and gitian and then checkout the qdp version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the qdp repository
...
To build QDP (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the qdp repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/qdp/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/QDPCRYPTO/QDP
...
./bin/gbuild --commit qdp=${COMMIT} --url qdp=${URL} ../qdp/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit qdp=${COMMIT} --url qdp=${URL} ../qdp/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit qdp=${COMMIT} --url qdp=${URL} ../qdp/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/qdp-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/qdp-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/qdp-build.assert
```
...
[qdp/gitian.sigs](https://github.com/qdp-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
